...
const MarketingDashboardClick: FC<ListProps> = (props) => {
    ...
    useEffect(() => {
        (async () => {
            if (requestList) {
                await getClickAllList();
                setRequestList(false);
            }
        })()
    }, [requestList])

    const getClickAllList = async () => {
        await marketingApi.getClickAll(search).then(res => {
            setClick([...res.lists].reverse());
...
export default MarketingDashboardClick;