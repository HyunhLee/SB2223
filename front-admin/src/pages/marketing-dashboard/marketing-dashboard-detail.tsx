...
const MarketingDashboardDetail: FC<ListProps> = (props) => {
    ...
    useEffect(() => {
        (async () => {
            if (requestList) {
                await getDetailAllList();
                setRequestList(false);
            }
        })()
    }, [requestList])

    const getDetailAllList = async () => {
        await marketingApi.getDetailAll(search).then(res => {
            setDetail([...res.lists].reverse());
            setStart(res.lists[0].date);
            setEnd(res.lists[res.lists.length-1].date);
...
export default MarketingDashboardDetail;