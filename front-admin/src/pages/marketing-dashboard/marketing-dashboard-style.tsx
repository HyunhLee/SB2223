...
const MarketingDashboardStyle: FC<ListProps> = (props) => {
    ...
    useEffect(() => {
        (async () => {
            if (requestList) {
                await getStyleAllList();
                setRequestList(false);
            }
        })()
    }, [requestList])

    const getStyleAllList = async () => {
        await marketingApi.getStyleAll(search).then(res => {
            setStyle([...res.lists].reverse());
...
export default MarketingDashboardStyle;