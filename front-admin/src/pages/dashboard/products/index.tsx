...
const applyFilters = (
  ...
  filters: Filters
): ProductModel[] => products.filter((product) => {
  if (filters.name) {
    const nameMatched = product.nameKo.toLowerCase().includes(filters.name.toLowerCase());
  ...
  if (filters.category?.length > 0) {
    const categoryMatched = filters.category.includes(product.nameKo);
  ...
  if (filters.status?.length > 0) {
    const statusMatched = filters.status.includes(product.nameKo);
...
const applyPagination = (
  ...
  page: number,
  rowsPerPage: number
): ProductModel[] => products.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
