...
const AiManual: NextPage = () => {
    const router = useRouter();
    const dataContext = useContext(DataContext);
    const isMounted = useMounted();

    const {styleRecommendId} = router.query;
    ...
    const getStyleRecommend = useCallback(async () => {
        try {
            const result = await styleApi.getStyleRecommend(styleRecommendId);
    ...
    useEffect(() => {
        if (styleRecommendId && styleRecommendId !== '0') {
            (async () => {
                await getStyleRecommend();
            })()
        }
    }, []);
    ...
    const addFileImageHandler = (imageFiles) => {
        ...
        styleRecommend.lookBookImage = imageFiles;
        const saveData = {...styleRecommend};
        let formData = new FormData();
        if(imageFiles.length > 0) {
            Object.keys(saveData).forEach(key => {
                if (key === 'lookBookImage') {
                    saveData[key].forEach((file: any) => {
                        formData.append('lookbookImage', file)
                    })
                } else {
            ...
            return axiosInstance.post(`/services/product/api/style-recommends/lookbook/inference`, formData, {
                headers: {
                ...
                let array = [];
                for(let i = 0; i < res.data.lookbookCategorys.length; i++) {
                    res.data.lookbookCategorys[i].key = `${i}`;
                    if(res.data.lookbookCategorys[i].category1 == 5 && res.data.lookbookCategorys[i].category2 == 22) {
                        array.unshift(res.data.lookbookCategorys[i]);
                    } else {
                        array.push(res.data.lookbookCategorys[i]);
                    }
                }
                setStyleRecommend({...styleRecommend, items: array, lookBookImageUrl: res.data.lookbookImageUrl});
            }).catch(err => {
                console.log(err);
            });
        }
    ...
    const saveHandler = async (): Promise<void> => {
        ...
        if (window.confirm('저장하시겠습니까?')) {
            ...
            let result = null;
            if (saveData.id) {
                result = await styleApi.putStyleRecommend(saveData);
            } else {
                result = await styleApi.postStyleRecommend(saveData);
            }
            if (result && result.id) {
