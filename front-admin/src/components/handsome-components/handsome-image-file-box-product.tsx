...
import {ImageInFormWidget} from "./image-widget";
import {ListManager} from "react-beautiful-dnd-grid";

const HandsomeImageFileBoxProduct = (props) => {

    const {header, imageUrlList, addFileImage, setImageUrlList} = props;
    ...
    const handleDrop = (newFiles: any): void => {
        newFiles.forEach((file, index) => {
            setFiles((prevFiles) => [...prevFiles, {imageUrl:URL.createObjectURL(file), listOrder:files.length + index + 1, size: file.size} ])
        })
    };
...
export default HandsomeImageFileBoxProduct;