...
const TotalPurchaseInCartDetail = (props) => {
  ...
  const handleChange = (prop: keyof FittingRoomCartSearchModel)=> (event: ChangeEvent<HTMLInputElement>) => {
    if (prop == 'productId' || prop == 'minPrice' || prop == 'maxPrice') {
  ...
  const excelDownload = useCallback(async () => {
    ...
    let test = [];
    await marketingApi.getCartPurchaseData({...search, size: itemsCount}
    ).then(res => {
    ...
    const excelHandler = {
      getExcelFileName:() => {
        return "피팅룸_기여_매출_"+ moment().format('YYYYMMDD')+'.xlsx';
      },
...
sx={{mt: 1, mb:2}}>
      ...
      <Box>
        <Grid sx={{m: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 1, marginBottom: 2}}>
