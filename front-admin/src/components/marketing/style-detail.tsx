...
import {defaultSearchStyle, SearchStyle, StyleModel} from "../../types/marketing-model/marketing-style-model";
...
const StyleDetail = (props) => {
    ...
    const url = 'https://partner-center-image.stage.stylebot.io/user-style/';

    const handleChange =
        (prop: keyof SearchStyle) => (event: ChangeEvent<HTMLInputElement>) => {
            if (prop == 'productId' || prop == 'styleId' || prop == 'productNo') {
    ...
    const getProduct = async () => {
        ...
        await marketingApi.getStyleProduct(query).then(res => {
            setLists(res.lists);
            setCount(res.count);
    ...
    const excelDownload = useCallback(async () => {
        ...
        let test = [];
        const result = await marketingApi.getStyleProduct({...search, size: count});
        result.lists.forEach((res) => {
            res.products.sort((a, b) => {return b.count - a.count}).map((r) => {
                return test.push({
                    styleAvatarImageUrl: res.styleAvatarImageUrl,
                    id: res.id,
                    thumbnailImageUrl: `${url}${r.thumbnailImageUrl}`,
                    productId: r.id,
                    productNo: r.productNo,
                    name: r.nameKo,
                    category: renderCategory(r.closetCategoryId),
                    brand: renderBrand(r.brandId),
                    count: r.count,
                    userId: res.userLogin,
                    createdDate: moment(res.createdDate).format('YYYY-MM-DD')
                })
            })
        });
        ...
        const excelHandler = {
            getExcelFileName:() => {
                return "MY_스타일_저장_수_"+ moment().format('YYYYMMDD')+'.xlsx';
            },
        ...
        ['이미지', '스타일ID', '상품 이미지', '상품ID', '상품번호', '상품명', '카테고리', '브랜드', '저장횟수', '유저ID', '등록일'].forEach((x, idx) => {
