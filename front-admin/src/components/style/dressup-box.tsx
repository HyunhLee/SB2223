...
const DressupBox = (props) => {
  const dataContext = useContext(DataContext);

  const {item, deleteStyle} = props;
  ...
  // 초기값 셋팅
  useEffect(() => {
    if (dataContext.CATEGORY_MAP) {
      if (item.category5 != null && item.category5 != 0) {
        setCategory(getDataContextValue(dataContext, 'CATEGORY_MAP', item.category5, 'path'));
      } else if (item.category4 != null && item.category4 != 0) {
        setCategory(getDataContextValue(dataContext, 'CATEGORY_MAP', item.category4, 'path'));
      } else if (item.category3 != null && item.category3 != 0) {
        setCategory(getDataContextValue(dataContext, 'CATEGORY_MAP', item.category3, 'path'));
      } else if (item.category2 != null && item.category2 != 0) {
        setCategory(getDataContextValue(dataContext, 'CATEGORY_MAP', item.category2, 'path'));
      } else if (item.category1 != null && item.category1 != 0) {
        setCategory(getDataContextValue(dataContext, 'CATEGORY_MAP', item.category1, 'path'));
      }
    }
