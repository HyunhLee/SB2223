...
import {defaultSearchDetailClick, DetailClickModel, SearchDetailClick} from "../../types/marketing-detail-click-model";
...
const DetailClickDetail = (props) => {
    ...
    const getDetailProduct = async () => {
        await marketingApi.getDetailProduct(search).then(res => {
            setLists(res.lists);
            setCount(res.count);
    ...
    const originalResultsExcelDownload = useCallback(async() => {
        if(count > 200){
            ...
            let img = '';
            const response = await marketingApi.getDetailProduct({...search, size: count});
            // @ts-ignore
            response?.lists.forEach((value) => {
                ...
                dataList.push({
                    ...
                    category: getCategoryForFile(value.product.closetCategoryId),
                    containCount: value.clickCounts,
                })
            })

            const excelHandler = {
                getExcelFileName: () => {
                    return "상품 상세 페이지 조회수_" + moment().format('YYYYMMDD') + '.xlsx';
                },

                getSheetName: () => {
                    return '상품 상세 페이지 조회수'
                },
            ...
            ['상품 이미지', '상품번호', '상품명', '브랜드', '카테고리', '상품조회수'].forEach((x, idx) => {
